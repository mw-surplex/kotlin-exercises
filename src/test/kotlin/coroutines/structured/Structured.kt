...
class StructuredTests {
    ...
    fun `FakeFactoryControl produces machines using producer`() {
        val perfectFactoryControl = FakeFactoryControl(machineProducer = StructuredTests::PerfectMachine)
        ...
        val failingFactoryControl = FakeFactoryControl(machineProducer = StructuredTests::FailingMachine)
    ...
    fun `Function creates a new machine every 800ms up to 5 and no more if they are all perfect`() = runTest {
        val control = FakeFactoryControl(machineProducer = StructuredTests::PerfectMachine)
    ...
    fun `Function creates a new machine every 800ms every time if all machines are failing`() = runTest {
        val control = FakeFactoryControl(machineProducer = StructuredTests::FailingMachine)
        setupFactory(control)
        for (i in 0..100) {
            assertEquals(i, control.createdMachines.size)
            delay(800)
